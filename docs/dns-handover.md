...
is sent separately rather than written here — Cloudflare hides it, and
...
`pluggzofficial.com` keeps working — anyone who types it lands in the right
...
unauthenticated — more likely to land in spam, and the domain can be spoofed by
...
`onsecureserver.net` and is set to `p=quarantine` — anything failing goes to
...
| DNS propagates | — | 15 min – 4 hrs after the change |
...
| Live on the domain | — | same day as the nameserver change |
